        ...
        public static byte[] Generate(List<ReportItem> Reports)
        {
            using (var Package = new ExcelPackage())
            {
                foreach (var Report in Reports)
                {
                    ...
                    for (; i < Report.CompetingModel.MaxRoundsCount; i++)
                    {
                    ...
                    for (; b < teamsCount; b++)
                    {
                        ...
                        //HARDCODED
                        string SumFormula = "SUM(" + FROM + ":" + TO + ")";
                        switch (Report.CompetingModel.MaxRoundsCount)
                        {
                            case 9:
                                for (byte sfi = 0; sfi < 2; sfi++)
                                {
                                    SumFormula += "-LARGE(" + FROM + ":" + TO + ", " + (sfi + 1) + ")";
                                }
                                break;
                            case 12:
                                for (byte sfi = 0; sfi < 3; sfi++)
                                {
                                    SumFormula += "-LARGE(" + FROM + ":" + TO + ", " + (sfi + 1) + ")";
                                }
                                break;
                            default:
                                //6
                                SumFormula += "-LARGE(" + FROM + ":" + TO + ", 1)";
                                break;
                        }
                        Sheet.Cells[TeamRowStart, SUMCol].Formula = SumFormula;
